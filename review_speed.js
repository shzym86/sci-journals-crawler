...
// 生成collection模型
const reviewSpeed = mongoose.model('ReviewSpeed', schema);

// 封装保存数据的函数
function upsertReviewSpeed(obj) {
  // 连接数据库
  ...
  // 将数据写入reviewspeeds集合中
  reviewSpeed.findOneAndUpdate(conditions, obj, options, (err, result) => {
    if (err) {
      throw err;
    }
  });
...
async function run(src) {
  ...
  for (let i = 0; i < quantity; i++) {
    ...
    let url = `https://journalinsights.elsevier.com/journals/${journal}/review_speed`;
    await page.goto(url, {
      timeout: 0,
      waitUntil: "domcontentloaded"
    });
    // 解析页面，结果返回为一个对象
    let SELECTOR = ".count3columns>tbody>tr:nth-child(1)>td";
    let result = await page.evaluate(($sel, issn) => {
      ...
      if (info.length > 0) {
        ...
        let firstSubmit = info[1].innerText;
        let finalSubmit = info[2].innerText;
        let dateCrawled = new Date().toLocaleString();
        // return `${issn}&${year}&${firstSubmit}&${finalSubmit}`;
        return {
          issn,
          year,
          firstSubmit,
